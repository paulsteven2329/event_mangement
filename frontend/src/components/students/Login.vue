  ...
  async login() {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        // Store student_id and email in localStorage
        localStorage.setItem('student_id', response.data.student_id);
        localStorage.setItem('student_email', response.data.data.email);  // Store email here
        console.log('Stored email in localStorage:', response.data.data.email); // Check if email is saved

        // Redirect to the profile page or dashboard
        this.$router.push('/DashboardForm');
      } else {
        alert('Login failed');
      }
    } catch (error) {
