<template>
  ...
  export default {
    ...
    data() {
      return {
        attendance: [],
        email: localStorage.getItem('email') || '',  // Assuming the email is saved in localStorage
        loading: false,
    ...
    methods: {
      async fetchAttendance() {
        ...
        try {
          const response = await axios.post('http://localhost:3000/student/attendance-by-email', {
            email: this.email, // Send the email instead of student_id
          });
